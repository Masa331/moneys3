  ...
  module Parsers
    class MoneyData
      ...
      def to_h
        ...
        hash[:seznam_cinnosti] = seznam_cinnosti.to_h if has? :SeznamCinnosti
        hash[:seznam_stredisek] = seznam_stredisek.to_h if has? :SeznamStredisek
        hash[:seznam_clen_dph] = seznam_clen_dph.to_h if has? :SeznamClenDPH
        hash[:seznam_uc_osnov] = seznam_uc_osnov.to_h if has? :SeznamUcOsnov
        hash[:seznam_predkontaci] = seznam_predkontaci.to_h if has? :SeznamPredkontaci
        hash[:seznam_predkontaci_de] = seznam_predkontaci_de.to_h if has? :SeznamPredkontaciDE
        hash[:seznam_kurz_listku] = seznam_kurz_listku.to_h if has? :SeznamKurzListku
        hash[:seznam_cen_hladin] = seznam_cen_hladin.to_h if has? :SeznamCenHladin
        hash[:seznam_firem] = seznam_firem.to_h if has? :SeznamFirem
        hash[:seznam_km_karta] = seznam_km_karta.to_h if has? :SeznamKmKarta
        hash[:seznam_zasoba] = seznam_zasoba.to_h if has? :SeznamZasoba
        hash[:seznam_skladu] = seznam_skladu.to_h if has? :SeznamSkladu
        hash[:seznam_typ_kusovnik] = seznam_typ_kusovnik.to_h if has? :SeznamTypKusovnik
        hash[:seznam_zak_kusovnik] = seznam_zak_kusovnik.to_h if has? :SeznamZakKusovnik
        hash[:seznam_zamestnancu] = seznam_zamestnancu.to_h if has? :SeznamZamestnancu
        hash[:seznam_zakazka] = seznam_zakazka.to_h if has? :SeznamZakazka
        hash[:seznam_fakt_prij] = seznam_fakt_prij.to_h if has? :SeznamFaktPrij
        hash[:seznam_fakt_vyd] = seznam_fakt_vyd.to_h if has? :SeznamFaktVyd
        hash[:seznam_int_dokl] = seznam_int_dokl.to_h if has? :SeznamIntDokl
        hash[:seznam_pok_dokl] = seznam_pok_dokl.to_h if has? :SeznamPokDokl
        hash[:seznam_bank_dokl] = seznam_bank_dokl.to_h if has? :SeznamBankDokl
        hash[:seznam_obj_prij] = seznam_obj_prij.to_h if has? :SeznamObjPrij
        hash[:seznam_obj_vyd] = seznam_obj_vyd.to_h if has? :SeznamObjVyd
        hash[:seznam_nab_prij] = seznam_nab_prij.to_h if has? :SeznamNabPrij
        hash[:seznam_nab_vyd] = seznam_nab_vyd.to_h if has? :SeznamNabVyd
        hash[:seznam_popt_prij] = seznam_popt_prij.to_h if has? :SeznamPoptPrij
        hash[:seznam_popt_vyd] = seznam_popt_vyd.to_h if has? :SeznamPoptVyd
        hash[:seznam_prijemka] = seznam_prijemka.to_h if has? :SeznamPrijemka
        hash[:seznam_vydejka] = seznam_vydejka.to_h if has? :SeznamVydejka
        hash[:seznam_dl_prij] = seznam_dl_prij.to_h if has? :SeznamDLPrij
        hash[:seznam_dl_vyd] = seznam_dl_vyd.to_h if has? :SeznamDLVyd
        hash[:seznam_prodejka] = seznam_prodejka.to_h if has? :SeznamProdejka
        hash[:seznam_prevodka] = seznam_prevodka.to_h if has? :SeznamPrevodka
        hash[:seznam_vyrobka] = seznam_vyrobka.to_h if has? :SeznamVyrobka
        hash[:seznam_inv_dokl] = seznam_inv_dokl.to_h if has? :SeznamInvDokl
        hash[:seznam_mezd] = seznam_mezd.to_h if has? :SeznamMezd
